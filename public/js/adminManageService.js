function Price() {
  ...
  for (i = 0; i < servicePrices.length; i++) {
    if (
      servicePrices[i]["Wash_Package_ID"] == x &&
      servicePrices[i]["Vehicle_Type"] == y
    ) {
...
function openViewServiceForm(i) {
  ...
  document.getElementById("serviceDesc").placeholder =
    pausecontent[i]["Description"];
  document.getElementById("ServiceName").innerHTML = pausecontent[i]["Name"];
  document.getElementById("editWashPackage").action =
    "/service/editWashPackage/" + pausecontent[i]["Wash_Package_ID"];
...
function openViewVehicleType(i) {
  ...
  document.getElementById("VehicleName").innerHTML =
    "Do you want to remove " + vehicles[i]["Vehicle_Type"] + "?";
...
function HideAdd() {
  ...
  document.getElementById("inputAddPrice").style =
    "display:inline-block; border-radius: 5px; width:50%;";
